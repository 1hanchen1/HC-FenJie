...
public class MainCommand implements CommandExecutor {
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if (command.getName().equals("epfj")) {
            if (args.length == 0) {
                sender.sendMessage("/epfj open 打开分解界面");
                sender.sendMessage("/epfj reload 重载配置文件");
                return true;
            }
            if (args[0].equals("open") && (sender instanceof Player) && sender.hasPermission("epfj.open")) {
